...
// Código opcional para adicionar ao seu App.jsx ou componente principal
/*
useEffect(() => {
  // Adicionar elementos de impulso neural
  const addNeuralImpulses = () => {
    for (let i = 0; i < 10; i++) {
      const impulse = document.createElement('div');
      impulse.className = 'neural-impulse';
      
      // Posicionar aleatoriamente
      impulse.style.left = `${Math.random() * 80 + 10}%`;
      impulse.style.top = `${Math.random() * 80 + 10}%`;
      
      // Atrasar animação para cada impulso
      impulse.style.animationDelay = `${Math.random() * 10}s`;
      
      // Alternar cores
      if (Math.random() > 0.5) {
        impulse.style.backgroundColor = 'rgba(0, 230, 118, 0.4)';
      }
      
      document.body.appendChild(impulse);
    }
    
    // Adicionar código binário em algumas áreas
    const binaryData = document.createElement('div');
    binaryData.className = 'binary-data';
    binaryData.style.left = '5%';
    binaryData.style.bottom = '10%';
    ...
    binaryData.textContent = binaryText;
    document.body.appendChild(binaryData);
    
    const binaryData2 = document.createElement('div');
    binaryData2.className = 'binary-data';
    binaryData2.style.right = '5%';
    binaryData2.style.top = '10%';
    
    let binaryText2 = '';
    for (let i = 0; i < 10; i++) {
      for (let j = 0; j < 15; j++) {
        binaryText2 += Math.random() > 0.5 ? '1' : '0';
      }
      binaryText2 += '\n';
    }
    binaryData2.textContent = binaryText2;
    document.body.appendChild(binaryData2);
  };
  ...
  // Limpar ao desmontar
  return () => {
    document.querySelectorAll('.neural-impulse, .binary-data').forEach(el => {
